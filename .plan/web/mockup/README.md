...
3. **Style reference** — deltas/exceptions from `../../design/style-guide.md` only (the shared doc, also used by mobile).
4. **Wireframe(s)** — one per state that's meaningfully different in layout, ASCII art in a fenced code block, using this screen's real content. The web app is always designed mobile-friendly (responsive), so include both a mobile-width and desktop-width wireframe where the layout differs.
5. **Interactions** — what each element does, validation rules, loading/disabled behavior, breakpoint behavior.
